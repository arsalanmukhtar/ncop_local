...
export class SourceLayerControl {
    ...
    preloadAllSources() {
        ...
        for (const categoryKey in ncop_menu_items) {
            ...
            for (const subcategoryKey in category) {
                const subcategory = category[subcategoryKey];
                const items = [subcategory.toggle, subcategory.temporal, subcategory.button, subcategory.dropdown].filter(Boolean);
                items.forEach(item => {
                    if (typeof item === 'object') {
                        for (const key in item) {
                            const config = item[key];
                            if (config && config.source && config.source.id && !sourcesSet.has(config.source.id)) {
    ...
    async addLayerByKey(layerKey, shouldLog = true) {
        ...
        if (!config.source || !config.layers) {
            console.warn(`⚠️ Layer "${layerKey}" missing source or layers configuration`);
            return false;
        }
    ...
    removeLayerByKey(layerKey, shouldLog = true) {
        ...
        if (!layerInfo) {
            if (shouldLog) console.warn(`⚠️ Layer "${layerKey}" is not currently active`);
            return false;
        }
        ...
        try {
            ...
            layersConfig.forEach((layerConfig) => {
                ...
                if (layer.type === 'symbol' && layer.layout && layer.layout['icon-image']) {
                    ...
                    if (!iconName.includes('/') && !iconName.endsWith('.webp') && !iconName.endsWith('.png')) {
                        ...
                        const tryAddLayer = () => {
                            if (this.map.getSource(sourceId)) {
                                ...
                                const layerKey = Object.keys(this.activeLayers).find(key => {
                                    ...
                                    return info && info.sourceId === sourceId && info.layerIds.includes(layerId);
                                });
                                if (!layerKey) {
                                        ...
                                        for (const subcategoryKey in category) {
                                            ...
                                            items.forEach(item => {
                                                if (typeof item === 'object') {
                                                    for (const key in item) {
                                                        const config = item[key];
                                                        if (config && config.source && config.source.id === sourceId) {
